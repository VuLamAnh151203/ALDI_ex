...
class BPRMF(tf.keras.Model):
    def __init__(self, user_num, item_num, args):
        ...
        self.user_emb = tf.keras.layers.Embedding(
            user_num,
            args.factor_num,
            embeddings_initializer=tf.keras.initializers.TruncatedNormal(stddev=0.01)
        )
    ...
    def call(self, u, i):
        ...
        return tf.reduce_sum(u_emb * i_emb, axis=-1)

    def bpr_loss(self, u, pos_i, neg_i):
        u_emb = self.user_emb(u)
        pos_emb = self.item_emb(pos_i)
        neg_emb = self.item_emb(neg_i)

        pos_score = tf.reduce_sum(u_emb * pos_emb, axis=-1)
        neg_score = tf.reduce_sum(u_emb * neg_emb, axis=-1)

        loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                logits=pos_score - neg_score,
                labels=tf.ones_like(pos_score)
            )
        )

        reg = self.reg_rate * (
            tf.nn.l2_loss(u_emb) +
            tf.nn.l2_loss(pos_emb) +
            tf.nn.l2_loss(neg_emb)
        )

        return loss + reg

    @tf.function
    def train_step(self, u, pos_i, neg_i):
        with tf.GradientTape() as tape:
            loss = self.bpr_loss(u, pos_i, neg_i)

        grads = tape.gradient(loss, self.trainable_variables)
        self.optimizer.apply_gradients(zip(grads, self.trainable_variables))
        return loss

    def get_user_rating(self, u_ids, i_ids):
        u_emb = self.user_emb(u_ids)
        i_emb = self.item_emb(i_ids)
        return tf.matmul(u_emb, i_emb, transpose_b=True)

    def get_embedding(self):
        return self.user_emb.weights[0], self.item_emb.weights[0]
