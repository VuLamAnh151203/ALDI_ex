...
#                                                 args.test_batch_us)
# exclude_val_overall = utils.get_exclude_pair_count(para_dict['pos_user_nb'],
#                                                   para_dict['overall_val_user'][:args.n_test_user],
#                                                   para_dict['overall_val_user_nb'],
#                                                   args.test_batch_us)
...
#                                                  args.test_batch_us)
# exclude_test_overall = utils.get_exclude_pair_count(para_dict['pos_user_nb'],
#                                                    para_dict['overall_test_user'][:args.n_test_user],
#                                                    para_dict['overall_test_user_nb'],
#                                                    args.test_batch_us)
...
# overall_res, _ = ndcg.test(model.get_ranked_rating,
#                           lambda u, i: model.get_user_rating(u, i, gen_user_emb, gen_item_emb),
#                           ts_nei=para_dict['overall_test_user_nb'],
#                           ts_user=para_dict['overall_test_user'][:args.n_test_user],
#                           item_array=para_dict['item_array'],
#                           masked_items=None,
#                           exclude_pair_cnt=exclude_test_overall,
#                           )
# timer.logging("overall recommendation result@{}: PRE, REC, NDCG: {:.4f}, {:.4f}, {:.4f}".format(
#     args.Ks[0], overall_res['precision'][0], overall_res['recall'][0], overall_res['ndcg'][0]))
...
#     f.write(' | ')
#     for i in range(len(args.Ks)):
#         f.write('%.4f %.4f %.4f ' % (overall_res['precision'][i], overall_res['recall'][i], overall_res['ndcg'][i]))
#     f.write('\n')
...
parser.add_argument('--beta', type=float, default=0.05)
parser.add_argument('--gamma', type=float, default=0.1)
parser.add_argument('--tws', type=int, default=0)
parser.add_argument('--freq_coef_M', type=float, default=4)
...
emb_path = os.path.join(dataset_path, f"{args.embed_meth}-{args.embed_loss}.npy")
emb = np.load(emb_path)
...
timer.logging(f'Load embeddings from {emb_path}')


# =====================
# Item frequency
# =====================
item_freq = np.ones((ITEM_NUM,), dtype=np.float32)
item_to_user_neighbors = para_dict['emb_item_nb'][para_dict['warm_item']]

for item_index, user_neighbor_list in zip(para_dict['warm_item'], item_to_user_neighbors):
    item_to_item_neighborhoods = para_dict['emb_user_nb'][user_neighbor_list]
    item_freq[item_index] = sum(1.0 / len(n) for n in item_to_item_neighborhoods)

x_expect = (len(train_data) / ITEM_NUM) * (1 / (len(train_data) / USER_NUM))
args.freq_coef_a = args.freq_coef_M / x_expect
...
for epoch in tqdm(range(1, args.max_epoch + 1)):
    ...
    if epoch >= args.val_start:
        ...
        en_user_emb = model.get_user_emb(user_emb)
        gen_item_emb = model.get_item_emb(
            content_data, item_emb,
            para_dict['warm_item'], para_dict['cold_item']
        )
